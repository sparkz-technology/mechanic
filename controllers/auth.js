...
export const createUser = async (req, res, next) => {
  try {
    const { username, name, phoneNumber, email, password, role } = req.body
    ...
    const newProfile = new Profile({
      ...
      bio: role == "mechanic" ? "I'm a mechanic" : "I'm a customer",
    })
...
export const forgotPassword = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return next(
        new AppError([{ field: "email", message: "User not found" }], 404)
      )
    }
